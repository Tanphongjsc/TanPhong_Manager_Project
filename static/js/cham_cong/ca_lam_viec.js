 ...
 */
class CaLamViecController extends BasePageManager {
    constructor() {
        super({
            tableBodyId: 'table-body-ca',
            searchId: 'search-ca',
            filterFormId: 'filter-ca',
            selectAllId: 'select-all-ca',
            bulkActionsId: 'bulk-actions-ca',
            ...
            onRenderRow: (item) => {
                ...
                // 3. Xử lý Ẩn/Hiện Checkbox & Nút Xóa
                // Nếu là CAHANHCHINH -> Trả về chuỗi rỗng '' (Không hiển thị gì cả)
                ...
                const deleteBtnHtml = isSystemDefault 
                    ? '' 
                    : `<button type="button" class="delete-btn text-red-600 hover:text-red-900 transition-colors" onclick="window.CaLamViecController.handleDelete(${item.id}, '${item.TenCa}')" title="Xóa">
