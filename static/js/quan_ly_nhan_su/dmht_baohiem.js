...
class BaoHiemManager extends BaseCRUDManager {
    constructor() {
        super({
            ...
            fillFormData: (data) => {
                const form = document.getElementById('baohiem-form');
                if (!form) return;
                form.querySelector('[name="TenBaoHiem"]').value = data.tenbaohiem || '';
                form.querySelector('[name="MaBaoHiem"]').value = data.mabaohiem || '';
    ...
    initTable() {
        this.tableManager = new TableManager({
            ...
            apiEndpoint: '/hrm/to-chuc-nhan-su/api/bao-hiem/list/',
            ...
            onRenderRow: (item) => {
                ...
                tr.innerHTML = `
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <button type="button" onclick="window.BaoHiemManager.openSidebar('edit', ${item.id})" class="text-blue-600 hover:text-blue-900 mr-3" title="Sửa">
                            <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" /></svg>
                        </button>
                        <button type="button" class="delete-btn text-red-600 hover:text-red-900" data-id="${item.id}" data-name="${item.tenbaohiem}" title="Xóa">
...
window.BaoHiemManager = new BaoHiemManager();
document.addEventListener('DOMContentLoaded', () => window.BaoHiemManager.init());